...
class Detector(nn.Module):
    ...
    def input_transform(self, image, annotations):
        ...
        #Gray scale
        cj = transforms.RandomGrayscale(p = 0.2)
        image = cj(image)

        #Flip image
        flip_image = random.randint(0,3)
        for ann in anns:
            if ann["category_id"] == 0 or ann["category_id"] == 1 or ann["category_id"] == 7 or ann["category_id"] == 8 or ann["category_id"] == 10 or ann["category_id"] == 14:
                flip_image = 0
